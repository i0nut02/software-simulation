...
void runBinary(const std::string& binaryPath, int count, int numServers, int numRequests) {
    for (int i = 0; i < count; ++i) {
        std::ostringstream command;
        command << binaryPath << " " << numServers << " " << numRequests << " &"; // Build the command to run the binary in the background
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " numCustomers numCustomersRequests numCustomerServers numCouriers numCouriersRequests numCouriersServers numSuppliers numSuppliersRequests numSuppliersServers" << std::endl;
        return EXIT_FAILURE;
    }

    // Parse command line arguments
    int numCustomers = std::stoi(argv[1]);
    int numCustomersRequests = std::stoi(argv[2]);
    int numCustomerServers = std::stoi(argv[3]);
    ...
    command3 << "../../monitor/bin/main " << numCustomers << " " << numCustomerServers << " " <<  0 << " " << 0 << " " << 0 << " " << 0 << " &";
    ...
    for (size_t i = 0; i < binaries.size(); i+=2) {
        runBinary(binaries[i], counts[i], counts[i+1], requests[i]);
    }
